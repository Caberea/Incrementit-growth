...
if (action1.scene = 1) && (potcount > 0) && (action1.attacking = 0){
	...
	maintext.battletext = "You chug down the Fury Potion, the somewhat spicy tincture making you feel a rush of strength throughout your body. Your Attack increases by " +string(counter_potFURY.buffvalue)+".";
	maintext.atk = maintext.atk + counter_potFURY.buffvalue;
	potcount--;
	skipcheck = random_range(0,99)
	if (counter_potTURNSKIP.turnskip > skipcheck){
	maintext.battletext2 = "Thanks to the improvements you've helped Elena make on the design of her potions, you're able to open and drink down the Fury Potion before the enemy has time to respond, letting you act again."
...
if (action1.scene = 13) && (maintext.gold > 68) && (maintext.scrying = false){
	maintext.scrying = true;
	maintext.gold = maintext.gold - 69;
	action6slot = "";
}

if (action1.scene = 9){
	if (maintext.onehandmode = true){
	maintext.onehandmode = false;	
	}
	else{
	maintext.onehandmode = true;	
	}
}
